...
**Quente vs. frio (economia de contexto).** Plan mode pré-carrega só **SETUP + ROADMAP + `CONTEXT.md`** — este último é o glossário de **linguagem ubíqua** do domínio, e a spec e o código herdam o vocabulário dele; termo afiado numa conversa de plan mode entra lá na mesma passada. Some `DEBITO-TECNICO` se a fase toca área com débito conhecido. **CHANGELOG, DECISOES e o texto cheio dos ADRs são frios** — ledgers que crescem sem teto; consulte sob demanda (`grep` pelo ID).
...
**Princípio do ponteiro:** **resuma sempre com ponteiro** — a linha resumida referencia onde está o detalhe (o ID da fase no SETUP-HISTORICO, o ID do ADR). Com ponteiro é compressão; sem ponteiro é perda.
