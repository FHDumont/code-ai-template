...
- **O fecho da fase é 100% do agente.** Aprovada a fase pelo dono (o marco de fim de fase já é o gate), o agente: roda a revisão de fecho → abre o PR (corpo curto: o que/por quê + critérios, referência à `F-xxx`) via `gh`/CLI do forge → confirma checks verdes → **integra por `gh pr merge --rebase` → apaga a branch (remota E local) → sincroniza o `main` local** (`git switch main && git pull`). O dono não toca no GitHub. Não deixe branch órfã nem `main` local defasado: ao terminar, `git branch` mostra só `main`, atualizado com o `origin/main`.
